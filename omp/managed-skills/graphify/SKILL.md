...
**MANDATORY: You MUST use the `task` tool here (default general-purpose worker, no `agent` override). Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not use the `task` tool you are doing this wrong.**
...
Dispatch all chunk subagents in a single `task` call with one `tasks[]` entry per chunk - this is the only way they run in parallel. One `tasks[]` batch with N entries, not N separate `task` calls.

**IMPORTANT - subagent type:** Omit the `agent` field (or use the default `task` worker). Do NOT use `scout` - it is read-only and cannot write chunk files to disk, which silently drops extraction results. The default worker has Write and Bash access which the subagent needs.

Concrete example for 3 chunks:
```
[tasks[] entry 1: files 1-15, default worker]
[tasks[] entry 2: files 16-30, default worker]
[tasks[] entry 3: files 31-45, default worker]
```
...
- If the file is missing, the subagent was likely dispatched read-only (e.g. `scout`) — print a warning: "chunk N missing from disk — subagent may have been read-only. Re-run with the default general-purpose worker." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure the default general-purpose `task` worker (not `scout`) is used.

Merge all chunk files into `.graphify_semantic_new.json`. **After each `task` call completes, read the real token counts from its result and write them back into the chunk JSON before merging** — the chunk JSON itself always has placeholder zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
