...
**CRITICAL**: When performing any phase, you MUST read and use relevant content from rule detail files. This copy is installed as the omp skill `aidlc`; resolve the rule details directory as `skill://aidlc/aws-aidlc-rule-details/` (read via the `read` tool, e.g. `skill://aidlc/aws-aidlc-rule-details/common/process-overview.md`). If a project overrides this by providing its own copy at one of the paths below, prefer that instead:
